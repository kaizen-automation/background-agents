...
run "a_configured_key_is_injected_into_modal" {
  ...
  assert {
    condition     = local.modal_llm_secret_values == { ANTHROPIC_API_KEY = "test-anthropic-key", CLAUDE_CODE_USE_BEDROCK = "", AWS_BEARER_TOKEN_BEDROCK = "", AWS_REGION = "", AZURE_API_KEY = "", AZURE_RESOURCE_NAME = "" }
